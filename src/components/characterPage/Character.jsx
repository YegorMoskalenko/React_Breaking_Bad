...
const Character = ({characterStrName}) => {
    ...
    const [favChars, setFavChars] = useState([...breakingBadState.favoritesCharacters])
    ...
    const removeCharacterFromFavChars = charIndex => {
        ...
        setFavChars(() => [...favCharsNewObj])
    }
    const removeCharacterFromFavorites = () => {
        return RootStore.breakingBad.SET_FAVORITES_CHARACTERS(favChars)
    }

    useEffect(() => {
        if(breakingBadState.favoritesCharacters.length < favChars.length){
            ...
            <div className="character">
                ...
                <div className="character__info anim-items">
                    <div className="add-to-favor">
                        {!favChars.includes(characterStrName)
                            ?   <button className="add-to-favor__btn" onClick={() => setFavChars(() => [...favChars, characterStrName])}>Add to favorites</button>
                            :   <div className="remove-from-favor__div">
                                    <p className="remove-from-favor__text">Already in favorites</p>
                                    <button className="remove-from-favor__btn" onClick={() => removeCharacterFromFavChars(breakingBadState.favoritesCharacters.indexOf(characterStrName))}>Remove from favorites</button>
                                </div>
                        }
                     </div>
